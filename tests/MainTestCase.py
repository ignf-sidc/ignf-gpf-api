from ignf_gpf_api.__main__ import Main
...
class MainTestCase(GpfTestCase):
    ...
    def test_parse_args_config(self) -> None:
        ...
        o_args = Main.parse_args(["config", "--file", "toto.ini"])
        self.assertEqual(o_args.task, "config")
        ...
        o_args = Main.parse_args(["config", "--file", "toto.ini", "--section", "store_authentification"])
        self.assertEqual(o_args.task, "config")
        self.assertEqual(o_args.file, "toto.ini")
        ...
        o_args = Main.parse_args(["config", "--section", "store_authentification"])
        self.assertEqual(o_args.task, "config")
        self.assertIsNone(o_args.file)
        self.assertEqual(o_args.section, "store_authentification")
        self.assertIsNone(o_args.option)
        ...
        o_args = Main.parse_args(["config", "--section", "store_authentification", "--option", "password"])
        self.assertEqual(o_args.task, "config")
        self.assertIsNone(o_args.file)
        self.assertEqual(o_args.section, "store_authentification")
