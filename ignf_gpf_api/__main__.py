...
class Main:
    ...
    def __init__(self) -> None:
        ...
        if not Path(self.o_args.config).exists():
            raise GpfApiError(f"Le fichier de configuration précisé ({self.o_args.config}) n'existe pas.")
    ...
    def __datastore(self) -> Optional[str]:
        ...
        if self.o_args.datastore:
            ...
            if not l_datastores:
                raise GpfApiError(f"Le datastore demandé '{self.o_args.datastore}' n'a pas été trouvé. Vérifier le nom indiqué.")
            # Et on renvoi l'id
        ...
        if self.o_args.section is not None:
            ...
            if self.o_args.option is not None:
                ...
                try:
                    ...
                    raise GpfApiError(f"La section '{self.o_args.section}' n'existe pas dans la configuration.") from e_no_section_error
                except configparser.NoOptionError as e_no_option_error:
                    raise GpfApiError(f"L'option '{self.o_args.option}' n'existe pas dans la section '{self.o_args.section}'.") from e_no_option_error
            else:
                ...
                try:
                    ...
                    raise GpfApiError(f"La section '{self.o_args.section}' n'existe pas dans la configuration.") from e_key_error
        else:
            ...
            if self.o_args.file is not None:
                ...
                try:
                    ...
                    raise GpfApiError(f"Impossible d'écrire le fichier {self.o_args.file} : non autorisé") from e_permission_error
            else:
                # Sinon on l'affiche
                with io.StringIO() as o_string_io:
